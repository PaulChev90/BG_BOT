    ...
    <div className="container">
      {warehouses.map((warehouse, index) => (
        <div key={index} className="warehouse">
          ...
          <p>{warehouse.phone}</p>
          {warehouse.additional_inf && <p><strong>Доп. информация:</strong> {warehouse.additional_inf}</p>}
          <button onClick={() => window.open(`https://yandex.ru/maps/?rtext=${warehouse.latitude},${warehouse.longitude}`)}>
            Построить маршрут
          </button>
        </div>
      ))}
...
ReactDOM.render(<WarehouseList />, document.getElementById('app'));
